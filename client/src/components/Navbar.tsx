  ...
  return (
    ...
    >
      ...
      </button>}
      {
        localStorage.getItem("auth-token")&&
        <>
          <div onClick={handleMenuOpen} className="rounded-full w-10 h-10 border border-[#4F00F6] flex justify-center align-middle">
            <img src={localStorage.getItem("pic")||"https://avatars.githubusercontent.com/u/1?v=4"} alt="profile" className="w-8 h-8 rounded-full my-auto cursor-pointer"/>
          </div>
        </>
      }
    </motion.nav>
      ...
      <div className="bg-transparent w-full absolute flex justify-end pr-10">
        ...
        className="bg-blue-400/10 backdrop-blur-md w-40 p-4 rounded-md cursor-pointer">
          <ul onClick={handleMenuOpen} className="text-[0.9rem]">
            ...
            <li className="hover:text-[#806ce4]" onClick={handleLogout}>Logout</li>
          </ul>
        </motion.div>
      </div>}
    </>
